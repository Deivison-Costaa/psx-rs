...
**0205 — 10.52 (timer lhu/lbu clear-on-read).** `read16` chama `region_read_byte` duas vezes
(uma por byte), então só trocar `peek32`→`read32` ali corrigiria `lbu` e quebraria `lhu`
(segundo byte veria o valor já limpo pelo primeiro) — precisou de um braço próprio em `read16`
pro intervalo de timers, igual já existe pra SPU/GPU, chamando `Timers::read32` uma única vez
por acesso de 16 bits. Bateria 4/4 + 1 equivalente, 2/2 controles. **Há uma segunda branch
paralela não mesclada** (`iter/0205-dicr-bus-error`) fechando 10.49 (DICR bus error) — vai
divergir de STATUS/achados/ROADMAP-fechado até reconciliar no merge, igual aconteceu com o
lote anterior. **Exceção vigente: o orquestrador implementa a escada**
(`docs/orquestracao.md`).

## Próxima tarefa

Fechar e mesclar a branch `iter/0205-dicr-bus-error` (10.49, PR #218 aberto). Depois,
continuar a lista de achados legado `10.x` em `docs/achados.md` (pedido do usuário: uma
iteração por achado, PR a cada ~10 itens, orquestrador faz os difíceis e delega os fáceis pro
trabalhador). ~11 dos 16 achados classificados como bugs reais de emulação ainda faltam — ver
`docs/achados.md` pras citações de spec. Candidatos: 10.45 (load shadow, delay slot —
CUIDADO, R1), 10.55/10.56/10.57 (CD-ROM), 10.83/10.85 (Rayman), 10.102/10.114/10.116 (timing
arquitetural, grandes — melhor pro orquestrador). **Silent Hill CONTINUA travado** (0201.1);
**artefato de triângulos no Crash precisa reconfirmação** pós-10.42 (0202.1). Achado 10.115 —
âncoras relativas nos rayman_* segue pendente (manutenção de teste, não bug de jogo).
