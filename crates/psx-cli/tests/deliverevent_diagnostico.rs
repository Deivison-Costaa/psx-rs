...
#[test]
fn trace_format_inclui_a1_na_saida() {
    ...
    let loop_addr: u32 = 0x8000_000C;
    let code = [
        0x3404_002A, // ori $a0, $zero, 0x2A
        0x3405_0077, // ori $a1, $zero, 0x77
    ...
    let exe_path = bins_dir().join("trace_a1.psexe");
    ...
    assert!(
        stderr.contains("a1($5)=0x00000077"),
        "o trace deve reportar o VALOR de $a1 (ori $a1,$zero,0x77) sob o rotulo a1($5); stderr:\n{}",
        stderr
    );
    assert!(
        stderr.contains("a0($4)=0x0000002A"),
        "o trace deve reportar o VALOR de $a0 (ori $a0,$zero,0x2A) sob o rotulo a0($4); stderr:\n{}",
        stderr
    );
    assert!(
        stderr.contains("v0($2)=0x00000099"),
        "o trace deve reportar o VALOR de $v0 (ori $v0,$zero,0x99) sob o rotulo v0($2); stderr:\n{}",
        stderr
    );
