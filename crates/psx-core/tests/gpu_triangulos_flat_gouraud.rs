...
fn gouraud_short_edge_interpola_entre_cores_diferentes() {
    ...
    assert_eq!(gpu.vram_pixel(1, 3), 0,
        "B8s: a aresta esquerda passa em x=1,5 na linha 3; o pixel x=1 fica fora");
    assert_eq!(gpu.vram_pixel(2, 3), 0x0534,
        "B8s: (2,3) R=248*0,65=161>>3=20, G=248*0,3=74>>3=9, B=248*0,05=12>>3=1 em 8 bits; \
         obtido 0x{:04X}", gpu.vram_pixel(2, 3));
    assert_eq!(gpu.vram_pixel(4, 7), 0x06A7,
        "B8s: (4,7) R=62>>3=7, G=174>>3=21, B=12>>3=1; obtido 0x{:04X}",
        gpu.vram_pixel(4, 7));
