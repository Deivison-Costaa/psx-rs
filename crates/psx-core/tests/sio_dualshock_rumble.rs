...
fn comando_4dh_devolve_o_mapa_antigo_e_grava_o_novo() {
    ...
    let r = respostas(&sio, &[0x01, 0x4D, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
...
fn metodo_novo_respeita_motores_trocados() {
    let sio = pad();
    entra_config(&sio);
    respostas(&sio, &[0x01, 0x4D, 0x00, 0x01, 0x00, 0xFF, 0xFF, 0xFF, 0xFF]);
    sai_config(&sio);
...
fn comando_42h_em_config_tambem_aciona_os_motores() {
    ...
    respostas(&sio, &[0x01, 0x42, 0x00, 0x01, 0x70, 0x00, 0x00, 0x00, 0x00]);
...
fn botao_analog_para_e_trava_os_motores() {
    ...
    assert_eq!(sio.rumble(), PARADO);
    respostas(&sio, &[0x01, 0x42, 0x00, 0x01, 0xFF, 0x00, 0x00, 0x00, 0x00]);
    assert_eq!(sio.rumble(), PARADO, "o mapa voltou a FFh: nada liga os motores");
}
...
fn botao_analog_depois_de_config_troca_5ah_por_00h() {
    ...
    let r = respostas(&sio, &[0x01, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
