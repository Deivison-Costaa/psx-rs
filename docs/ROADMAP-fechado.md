...
## Fechado na limpeza de 2026-09-23 (iteração 0241)
- [x] 10.52 `lhu`/`lbu` no modo do timer nao limpa bits 11/12 (0118) — fechado: lhu/lbu do modo limpam os bits 11/12 (aa0ac58, PR #249)
- [x] 10.64 `evcb_descritor_mapeia_para_spec_correto` custa 447 s dos 449 s sob nextest (0140) — fechado: nextest do workspace inteiro roda em ~9 s (medido em 2026-09-23)
- [x] 10.68 `cdrom_evento_kernel`: 150 M passos sem saida antecipada em 2 testes (0144) — fechado: nextest do workspace inteiro roda em ~9 s (medido em 2026-09-23)
- [x] 10.83 Rayman: ~89/660 IRQ0 sem hook (0158) — fechado: Rayman roda e joga; os 12 jogos passam na suite contra o DuckStation (PR #256)
- [x] 10.85 Rayman: laco `0x801B9574` espera `[0x801CF2CC]>=2` (0159) — fechado: Rayman roda e joga; os 12 jogos passam na suite contra o DuckStation (PR #256)
- [x] 10.103 GetlocL/GetlocP stub; disc-swap exige tray scriptavel (06-cdrom.md L1052) (0175) — fechado: GetlocL/GetlocP de verdade (d71b973) e troca de disco scriptavel com --swap-disc/--open-lid (48dc5f3, PR #250)
- [x] 0184.1 MDEC: 35 de 512 palavras do gabarito divergem em 1 passo de 5 bits (0184) — fechado: MDEC exato, todos os oraculos mdec em 0 (3c82c23, PR #255)
- [x] 0188.1 XA reamostrado por vizinho mais proximo; o hardware usa zigzag de 25 pontos (0188) — fechado: XA reamostrado pelo zigzag de 25 pontos (c813406)
- [x] 0193.2 GPUSTAT bit 21 (display 24bpp) nunca lido: FMV/MDEC exibida listrada (0193) — fechado: display de 24bpp le tres bytes por pixel (PR #241)
- [x] 0193.4 CPU cobra 1 ciclo/instrucao sem custo de RAM/ROM e GPU desenha em 0 ciclos: jogo de 30 fps roda a 60 (0193) — fechado: custo por instrucao medido contra a spec com 0,04% de erro; escada de timing fechada na 0217
- [x] 0193.7 GPU: texture flip E1 bits 12/13 e GP1(10h) GPU-info ausentes (0193) — fechado: X/Y-Flip do E1h (8beb7cc) e GP1(10h) (2634162)
- [x] 0202.1 Crash Bandicoot: triangulos aparecem e somem rapido durante o jogo (relato do usuario, nao reproduzido por medicao ainda) — suspeita: ordem de desenho/pintor sem Z-buffer, ou lixo de VRAM entre frames; precisa de dump-vram-every com passo fino pra flagrar o frame do artefato (0202) — fechado: pisca-pisca do Crash era o GTE sob interrupcao (PR #254)
- [x] 0208.4 CTR trava esperando RAM 0x8007DD9C (contador de vsync do proprio jogo) incrementar; IRQ0/VBlank dispara e e reconhecido corretamente (I_STAT/I_MASK confirmados via watch-mem), mas o elo IRQ->contador do jogo nunca executa — handler encadeado ou EvCB especifico do CTR nao identificado (0208) — fechado: CTR chega a corrida; os 12 jogos passam na suite contra o DuckStation (PR #256)
- [x] 0214.3 Tekken 3 (atualiza 0208.1): a trava antiga de dupla-leitura do Timer 1 esta resolvida (fix 0208 + degraus 1-6) — o jogo anima ate a tela cheia da PlayStation (dumps de VRAM distintos aos passos 50M/100M/150M) e entao CONGELA ali por 250M passos (62% do orcamento de 400M), com IRQ2 de CD-ROM disparando continuamente a cada ~20-21 mil ciclos e o PC circulando por ~20 enderecos em faixas 0x80083Bxx/0x800859xx/0x8008F5xx/0x80091xxx sem produzir frame novo — travamento novo, distinto do antigo, com cara de espera de streaming/CD-ROM (0214) — fechado: Tekken 3 faz luta completa no Arcade; os 12 jogos passam na suite contra o DuckStation (PR #256)
- [x] 0214.4 RE2 (atualiza 0208.1): mesma trava antiga resolvida — o jogo inicializa a lib do pad (`PS-X Control PAD Driver Ver 3.0`), anima ate ~150-200M passos e entao trava num laco em 0x80081A54-0x80081AB0 (dentro do executavel do jogo, nao kernel): um contador de retentativas decrescente guarda um segundo bloco que compara RAM 0x800A5110 (global, cresce de verdade: 0x8C aos 200M passos -> 0x3B2 aos 400M) contra um alvo, sem alcanca-lo dentro do orcamento testado. Padrao "espera contador atingir alvo com timeout de retentativa" que 0193.4 e a suspeita original do usuario ja previam — primeira vez caracterizado com PC e endereco exatos (0214) — fechado: RE2 chega ao gameplay com o Leon; os 12 jogos passam na suite contra o DuckStation (PR #256)
- [x] 0214.6 CTR (confirma 0208.4 inalterado): os degraus 1-6 nao mudam nada. A unica escrita nova em RAM 0x8007DD9C (passo 160.575.277, valor 0xA4A000EE, pc de BIOS 0xBFC06618) nao parece incremento genuino de contador (valor nao sequencial, parece bytes de uma copia em bloco) e fica congelada pelo resto dos 400M passos; VRAM tambem congela a partir de ~150M passos. IRQ0/VBlank seguem com cadencia saudavel (583 bordas de subida, ~669K passos de intervalo medio, confirmado no orcamento inteiro) — a falha nao e de entrega de hardware, e' na cadeia software IRQ->contador do proprio jogo, ainda nao isolada; nenhum degrau da escada de timing CPU/barramento (1-9) deveria afetar isso (0214) — fechado: CTR chega a corrida; os 12 jogos passam na suite contra o DuckStation (PR #256)
- [x] 0227.1 (custo do 10.52, agora medido) O 10.52 ("`lhu`/`lbu` no modo do timer nao limpa bits 11/12") tem gabarito de hardware que reprova por causa dele: `spu/memory-transfer` falha `testDMAWriteTiming:255` e `testDMAReadTiming:288`. Mecanismo: `Bus::read16` e `&self`, cai em `region_read_byte` -> `Timers::peek32` (sem efeito colateral por construcao), enquanto `read32` usa `Timers::read32`, que limpa. O helper `measure()` do ps1-tests (`common/measure.hpp`, Timer2 em SystemClock/8) faz `resetTimer()` -> `read16(mode)` contando com a limpeza do latch, ve o bit12 velho em `timerDidOverflow()` e soma 0xFFFF ao valor lido: measuredCycles vai a ~524.280 contra o teto de 18.022. `Timers::write32` no offset 0x4 ainda preserva 0x7C00 (bits 10-14), entao nem o `initTimer` limpa. Consertar exige efeito colateral em `read16`/`read8`, hoje `&self` (0227) — fechado: consequencia do 10.52, consertada junto (aa0ac58, PR #249)
- [x] 0227.3 `mdec/4bit.exe` e `mdec/8bit.exe` imprimem `mdec_readDecodedDma timeout`, mas NAO por defeito nosso: o helper `mdec_readDecodedDma` (`common/mdec.cpp`) usa `BS=0x20` e calcula `BC=(bytes/4)/BS`, que para os 0x20 bytes desses dois testes da `8/32 = 0` por divisao inteira — BCR sai `0x00000020` (BA=0), e a spec manda ler BA=0 como 0x10000 blocos. O `psx.log` empacotado NAO serve de gabarito nessa linha: foi capturado de um build anterior do teste, com `blockSize=0x8` (visivel no proprio log), onde BC=1. Os pixels decodificados batem com o hardware exceto 2 bytes de arredondamento do IDCT (0227) — fechado: nao e defeito nosso: o helper do teste calcula BA=0

## Abandonado na limpeza de 2026-09-23 (iteração 0241): processo fora do foco
- [x] 10.2 Passe de compatibilidade (bugs viram itens 10.x) — abandonado: processo/ferramenta fora do foco
- [x] 10.28 Tabela por registro nos docs vs `.resultado`: 0071 errou 3/9, 0038 inflou 2 — abandonado: processo/ferramenta fora do foco
- [x] 10.29 `dma_dpcr_gate.rs:141`: `assert_ne!` como unica assercao de uma correcao — abandonado: processo/ferramenta fora do foco
- [x] 10.25 `unwrap_or` de caminho nos meta-testes silencia `strip_prefix` que falha (10.12) — abandonado: processo/ferramenta fora do foco
- [x] 10.17 `mutantes.ps1` recusa arvore suja; permitir `docs/mutantes/*.mut` — abandonado: processo/ferramenta fora do foco
- [x] 10.24 Job `scoreboard` da CI sai VERDE medindo zero (0072) — abandonado: processo/ferramenta fora do foco
- [x] 10.26 Nenhum dos 9 testes de `ci_scoreboard.rs` afirma que o job mede algo — abandonado: processo/ferramenta fora do foco
- [x] 10.27 Placar local (gitignored) e o unico com veredito real e nao e versionado — abandonado: processo/ferramenta fora do foco
- [x] 10.18 Nada torna `arquivada:` caro: 0052 e 0059 largaram 17 registros, 12 casavam — abandonado: processo/ferramenta fora do foco
- [x] 10.33 `scripts/mutantes.ps1` so roda `cargo test -p psx-core`; outros crates a mao — abandonado: processo/ferramenta fora do foco
- [x] 10.34 Nenhum meta-teste reprova `#[test]` sem assercao (T10 da 0080 passou assim) — abandonado: processo/ferramenta fora do foco
- [x] 10.35 `mutantes.ps1` grava nome qualificado, `mutation_battery` busca fn literal — abandonado: processo/ferramenta fora do foco
- [x] 10.40 `mutantes.ps1` so casa ancora em LF; CRLF da 'encontrada 0 vez(es)' — abandonado: processo/ferramenta fora do foco
- [x] 10.42b Manifesto trata `#` como comentario em `@@DE`/`@@PARA` (alvo `.md`) — abandonado: processo/ferramenta fora do foco
- [x] 10.44 Manifesto com alvo em documento vivo (STATUS.md) envelhece sempre — abandonado: processo/ferramenta fora do foco
- [x] 10.46 `justificativa:` do equivalente nao aceita continuacao de linha — abandonado: processo/ferramenta fora do foco
- [x] 10.58 mutantes.ps1 so roda psx-core; alvo por crate (invariante 29) (0125) — abandonado: processo/ferramenta fora do foco
- [x] 10.66 Meta-teste nao reexecuta bateria antiga: `.resultado` mente (0143) — abandonado: processo/ferramenta fora do foco
- [x] 10.63 `-ContinueBranch` diz "reprovado na revisao" apos travamento (0140) — abandonado: processo/ferramenta fora do foco
- [x] 10.65 Revisor isolado em /tmp nao le o repo nem confere citacao (0141) — abandonado: processo/ferramenta fora do foco
- [x] 10.71 `mutantes.ps1`: duas ramificacoes `teste` no mesmo switch (0146) — abandonado: processo/ferramenta fora do foco
- [x] 10.77 Trabalhador inventa metrica em vez de drenar metrics-pending.csv (0150) — abandonado: processo/ferramenta fora do foco
- [x] 10.78 oc-iter.ps1: exit 0 sem commits vira ok (0151) — abandonado: processo/ferramenta fora do foco
- [x] 10.107 `CLAUDE.md` lista `perf`/`ci` mas o commit-lint so aceita 6 tipos (0177) — abandonado: processo/ferramenta fora do foco
- [x] 10.115 Provas do Rayman fixam passo absoluto: melhoria legitima reprova (0174) — abandonado: processo/ferramenta fora do foco
- [x] 0181.1 `docs/relatorio.md` e `docs/orquestracao.md` ainda descrevem o ROADMAP unico (0181) — abandonado: processo/ferramenta fora do foco
- [x] 0190.1 `gte_fuzz_hardware` cobre 22 dos 27 comandos; NCLIP/DPCS/... sem caso de fuzz ficam so nos testes unitarios (0190) — abandonado: processo/ferramenta fora do foco
- [x] 0198.1 R3 "sem I/O" nao tem teste que varra src/; `bus.rs` tem 3 `eprintln!` de sonda dentro do core (0198) — abandonado: processo/ferramenta fora do foco
- [x] 0198.2 Justificativa da allowlist (`purity.rs`: "save states") mais estreita que o uso real de serde em `app/` (0198) — abandonado: processo/ferramenta fora do foco
- [x] 0198.3 `app/saves.rs` a 5,5% de comentario, acima do alvo de 5% da R7 (0198) — abandonado: processo/ferramenta fora do foco
