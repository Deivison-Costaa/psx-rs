...
- [x] 0222.1 As FMVs nao estavam "granuladas": a decodificacao MDEC->DMA1->VRAM estava certa e `Gpu::framebuffer()` e' que lia a area de display sempre como 15bpp, ignorando GPUSTAT.21 (Display Area Color Depth, docs/reference/03-gpu.md L1019 e L1279-1285). Como FMV e' o unico uso real do modo 24bpp, tudo que o app desktop (e o `--vram-to-png`) mostrava durante um video era ruido. Medido com GPUSTAT nos dumps de Silent Hill: bit21=1 nos dumps 10-12. Corrigido (0222)
- [ ] 0222.2 Oraculo `mdec/frame` 15bpp: a ultima das 20 chamadas `mdec_readDecoded(0x1e00 bytes)` nunca retorna, mesmo com 300M passos, embora o MDEC ja tenha as 38400 palavras (300 macroblocos) na fifo — as 36480 primeiras saem e a leitura da ultima tira de 16px trava antes do primeiro `pop`. Resultado: 2112 pixels da coluna de macrobloco 19 (x=304..319) divergem do retrato de hardware. O jogo espera algo no status que o nosso MDEC1 nao entrega (candidatos nao medidos: bit30 "Data-In Fifo Full" nunca setado; contador "parameter words remaining" em 0xFFFF fora de comando) (0222)
- [ ] 0222.3 Oraculo `mdec/frame` 15bpp, fora da tira perdida do 0222.2: 11653 pixels saem exatamente um degrau de 5 bits abaixo/acima do hardware e outros ~1660 divergem mais — arredondamento do `real_idct_core`/`yuv_to_rgb` (a propria spec, docs/reference/09-mdec.md, admite nao saber o arredondamento exato). Invisivel a olho nu na FMV; so aparece na comparacao pixel a pixel (0222)
