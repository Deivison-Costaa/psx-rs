...
- [ ] 0227.3 `mdec/4bit.exe` e `mdec/8bit.exe` imprimem `mdec_readDecodedDma timeout`, mas NAO por defeito nosso: o helper `mdec_readDecodedDma` (`common/mdec.cpp`) usa `BS=0x20` e calcula `BC=(bytes/4)/BS`, que para os 0x20 bytes desses dois testes da `8/32 = 0` por divisao inteira — BCR sai `0x00000020` (BA=0), e a spec (L76) manda ler BA=0 como 0x10000 blocos. O `psx.log` empacotado NAO serve de gabarito nessa linha: foi capturado de um build anterior do teste, com `blockSize=0x8` (visivel no proprio log), onde BC=1. Os pixels decodificados batem com o hardware exceto 2 bytes de arredondamento do IDCT (0227)
