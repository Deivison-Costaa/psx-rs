...
- [ ] 0227.1 amidog/gte OFFICIAL>TIMING: 9 dos 22 opcodes (SQR, NCLIP, AVSZ3, GPF, GPL, DPCS, INTPL, DCPL, MVMVA) ficam VERMELHOS e a celula **nao depende do custo do comando** — varrendo `Gte::command_cycles` desses opcodes por 0,1,2,3,4,6,7,8,9,10,11,12,40,60 a matriz sai identica. Controle positivo: quebrar o custo de RTPS/CC/AVSZ4/OP deixa exatamente essas 4 vermelhas, entao a coluna TIMING **e** sensivel a custo e os 13 verdes sao reais. Hipotese "o GTE nao cobra ciclo por comando" REFUTADA: a tabela existe desde 28132ad, bate com os cabecalhos de `07-gte.md` (teste `gte_custo_comando.rs`, 22 citacoes) e agora tambem com `18-gte-pipeline-timings.md`, que mede o mesmo valor em SCPH-5501 real. Sonda no proprio EXE: a medicao e um laco cronometrado pelo Timer 2 (`sh 0x1124(t5)` zera, `lhu 0x1120(t5)` le) em 12 sitios auto-modificaveis em 0x8004F490-0x8004F72C, com K=0/1/4/7 nops entre o `cop2` e um `cfc2 r31`/`mfc2 r25`; nosso valor por iteracao e exatamente `6+K+max(0,N-K)` no grupo cfc2, certo para os 13 verdes. Como o probe e o MESMO codigo para os 22 opcodes e so o imm25 muda, e o custo nao move a celula, o veredito desses 9 nao sai desse laco — descobrir de onde sai ANTES de mexer em qualquer numero (0227)
