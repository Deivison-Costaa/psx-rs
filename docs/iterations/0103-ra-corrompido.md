...
| 4 | ferramenta | Que consertar `cpu.rs` só afetasse os manifestos desta iteração | Duas âncoras alheias envelheceram de uma vez: a **0055/m4** (`self.cop0[14] = self.pc;`) porque meu `if/else` mudou a indentação dela, e as **oito da 0102** porque o alvo era o `STATUS.md`, que este handoff reescreve | `mutation_anchors.rs` reprovou no `cargo test --all`. A 0055 foi reancorada e a bateria dela **rodou de novo** (5/5); a 0102 foi arquivada com motivo. Item 10.44: manifesto sobre documento vivo envelhece na iteração seguinte, **sempre** |
